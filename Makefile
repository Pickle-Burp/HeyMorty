...
OUT		= hey_morty
CC		= gcc
FLAGS	= -g -c -Wall -Wextra -lfftw `pkg-config --cflags glib-2.0` `pkg-config --cflags gtk+-3.0` `pkg-config --cflags json-c`
LFLAGS	= -lm -lcurl -ltensorflow -lfftw `pkg-config --libs glib-2.0` `pkg-config --libs gtk+-3.0` `pkg-config --libs json-c`
